...
# Use downsampled data for CPU inference (set to True if running on CPU with limited memory)
# Run `python downsample_data.py` first to create the _small files
USE_DOWNSAMPLED = not torch.cuda.is_available()  # Auto-detect: use small data on CPU
DATA_SUFFIX = "_small" if USE_DOWNSAMPLED else ""
...
class AIService:
    ...
    def load_resources(self):
        ...
        if USE_DOWNSAMPLED:
            logger.info(f"  → Using DOWNSAMPLED data (suffix: '{DATA_SUFFIX}') for CPU inference")
        dem_path = STATIC_DIR / f"HoChiMinh_DEM{DATA_SUFFIX}.tif"
        manning_path = STATIC_DIR / f"HoChiMinh_Manning{DATA_SUFFIX}.tif"
        init_cond_path = STATIC_DIR / f"HoChiMinh_initial_condition{DATA_SUFFIX}.tif"
        ...
        with rasterio.open(dem_path) as src:
            ...
            self.dem_shape = dem.shape  # (H, W)
            logger.info(f"  → DEM shape: {dem.shape}, CRS: {src.crs}")
            logger.info(f"  → DEM bounds (native): {self.dem_bounds}")
            logger.info(f"  → DEM value range: [{np.min(dem):.2f}, {np.max(dem):.2f}]")
            
            # Calculate WGS84 bounds for API response
            if src.crs and src.crs != CRS.from_epsg(4326):
                transformer = Transformer.from_crs(src.crs, "EPSG:4326", always_xy=True)
                lon_min, lat_min = transformer.transform(self.dem_bounds.left, self.dem_bounds.bottom)
                lon_max, lat_max = transformer.transform(self.dem_bounds.right, self.dem_bounds.top)
                self.wgs84_bounds = {
                    "north": lat_max, "south": lat_min,
                    "east": lon_max, "west": lon_min,
                    "center": {"lat": (lat_max + lat_min) / 2, "lon": (lon_max + lon_min) / 2}
                }
                logger.info(f"  → WGS84 bounds: N={lat_max:.4f}, S={lat_min:.4f}, E={lon_max:.4f}, W={lon_min:.4f}")
            else:
                # Already WGS84
                self.wgs84_bounds = {
                    "north": self.dem_bounds.top, "south": self.dem_bounds.bottom,
                    "east": self.dem_bounds.right, "west": self.dem_bounds.left,
                    "center": {"lat": (self.dem_bounds.top + self.dem_bounds.bottom) / 2,
                               "lon": (self.dem_bounds.right + self.dem_bounds.left) / 2}
                }
    ...
    def resize_rain_to_dem(self, rain_npy, rain_meta=None):
        """
        Resize/reproject rainfall array to match DEM grid.
        rain_npy: 2D numpy array of rainfall
        rain_meta: optional rasterio profile with CRS and transform
        """
        target_h, target_w = self.dem_shape
        
        # If shapes already match, return as-is
        if rain_npy.shape == (target_h, target_w):
            return rain_npy
        
        logger.info(f"    • Resizing rain from {rain_npy.shape} to {(target_h, target_w)}")
        
        # Simple bilinear resize using numpy (fast, no CRS needed)
        from scipy.ndimage import zoom
        
        zoom_h = target_h / rain_npy.shape[0]
        zoom_w = target_w / rain_npy.shape[1]
        
        resized = zoom(rain_npy, (zoom_h, zoom_w), order=1)  # order=1 is bilinear
    ...
    def process_incoming_rain(self, rain_npy):
        ...
        if rain_npy.shape != self.dem_shape:
            logger.warning(f"  ⚠ Shape mismatch! Rain {rain_npy.shape} vs DEM {self.dem_shape}")
            rain_npy = self.resize_rain_to_dem(rain_npy)
            logger.info(f"  → Resized rain shape: {rain_npy.shape}")
...
def get_prediction():
    try:
        ...
        # Construct Metadata (use bounds from DEM)
        # Ho Chi Minh City approximate bounds (will be overwritten by actual DEM bounds)
        if hasattr(service, 'wgs84_bounds') and service.wgs84_bounds:
            bounds = service.wgs84_bounds
        else:
            # Fallback Ho Chi Minh bounds (approximate)
            bounds = {
                 "north": 10.88, "south": 10.65, "east": 106.88, "west": 106.55,
                 "center": {"lat": 10.76, "lon": 106.71}
            }
